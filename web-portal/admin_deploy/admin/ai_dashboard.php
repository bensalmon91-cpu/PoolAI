...
try {
    $result = $pdo->query("SELECT 1 FROM ai_questions LIMIT 1");
    if ($result === false) {
...
// Wrap all queries in try-catch to prevent 500 errors
try {
...
")->fetchAll(PDO::FETCH_KEY_PAIR);
    ...
    // Database error (likely missing tables), redirect to setup
    header('Location: ai_setup.php');
    exit;
}
