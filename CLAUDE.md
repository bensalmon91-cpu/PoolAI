...
**Last Updated:** 2026-08-29 · **Pi software current:** v6.11.25
...
- **Pi software (released):** v6.11.25 (fixed a 502 on the "view local controller" proxy button when opened through `pool.swanwood.app` behind Cloudflare Access: `proxy.py`'s `proxy_ui()` forwarded the incoming request's `Cookie` header — including Cloudflare Access's ~1.1-1.4KB `CF_Authorization` cookie — straight to the downstream Ezetrol controller, which has only a ~1.5KB header buffer and rejected the request outright. Direct-LAN access (no Cloudflare cookie) always worked, which is why it went unnoticed. Fixed by filtering the upstream headers through a new `_upstream_headers()`/`_UPSTREAM_DROP_HEADERS` helper that drops `Cookie`, the remaining RFC 7230 hop-by-hop headers, and any `CF-*`/`X-Forwarded-*` header before forwarding to the controller — the Pi's own session/cookie handling is untouched. Reproduced + verified with curl (502→200 with an oversized `Cookie` header) before and after. Hotfixed live on Swanwood 2026-08-29, then shipped as a proper release via `scripts/publish_update.py` and applied through the normal update channel.). Earlier: v6.11.24 (a 4-agent review of the v6.11.23 fix caught real follow-on issues, all fixed + verified with 67/67 existing + 8 new tests passing on real hardware: the deadlock fix had *unmasked* a `remount_ro()` bug that could take root read-only on every boot; `blkid` failures were indistinguishable from "confirmed blank" and could trigger a wrongful `mkfs.ext4`; `migrate_data()`'s unguarded `mv` could silently nest the SD backup; `start_services()` failures never reached the UI's status file; `chromium_scale_factor` was read with no bounds check where it mattered; `clone_prep.sh` had a second, drifted copy of the kiosk autostart, now installed from the single template instead.). Earlier: v6.11.23 (fresh-install of a new independent pool site on a 10" official touchscreen Pi ("10inch", `10.0.30.148`) surfaced a fleet-wide latent boot deadlock in the External Storage feature: `usb_data_mount.sh`'s `find_usb_device()` logged via a function whose stdout was captured by the caller (`USB_DEVICE=$(find_usb_device)`), so "no device found" still returned non-empty log text, tripping the script into a full migration against a directory that was never actually mounted; separately, `start_services()` called a *blocking* `systemctl start` on services the unit's own `Before=` ordering was waiting on it to finish first — a guaranteed self-deadlock on any boot with no USB stick attached, independent of the first bug. Both fixed (`log()` now writes to stderr; `mount_usb()` checks the actual mount exit code; service starts use `--no-block`). Hotfixed live on Swanwood 2026-07-30 (dormant there — service was present but never enabled) and 10inch (was actively hanging every boot). Also added a `chromium_scale_factor` setting (same pattern as `screen_rotation`) so the kiosk browser can compensate for high-DPI touchscreens — 10inch's 10" panel (~226dpi) is set to 1.5x to keep touch targets close to the 7" screen's (~132dpi) physical size.). Earlier: v6.11.22 (WiFi drops root-caused to a zombie `poolaissistant_ap_manager.service` — "removed" in v6.11.2's changelog, but the retirement only lived in `install_services.sh`, which never re-runs on an already-deployed Pi; watchdog.py was also actively resurrecting it every ~5 min via its critical-services list, likely why manual stops never stuck. Fully removed + self-healed fleet-wide. USB stick incident traced to the real "External Storage" feature: a synchronous 180s-timeout copy of a live 6+GB DB left an orphaned background copy and a dangling symlink; feature rewritten to stop services first, verify copy integrity, and report live progress. LSI history was silently broken since it was first built — `lsi_result.get(...)` on a dataclass raised `AttributeError` on every calculation, swallowed by a broad except — fixed, plus a matching connection-leak bug in `lsi_history.py`; maintenance page rebuilt around LSI with 6 duplicate entry points collapsed to 2 and an internalized (no AI/API call) interpretation module. Settings: 3 dead routes removed, the `storage_max_mb` "500" fallback bug fixed in 7 places, a Portal Sync section added surfacing upload status/errors, validation de-duplicated into `persist.py`. Deployed + fully verified on Swanwood 2026-07-27; the largest remaining piece (settings blueprint extraction) deliberately deferred to its own follow-up per the plan's own risk ordering.). Earlier: v6.11.21 (data retention finally *runs* — the v6.11.19 timer fired nightly but the script died at its 30-min timeout every run on non-indexable per-hour-group DELETEs; thinning rewritten set-based with day slices, a wall-clock budget and resume cursors. Journald persistence root-caused too: RPi OS's vendor `40-rpi-volatile-storage.conf` lexically overrode our drop-in; ours is now `99-`, fixed live on Swanwood + self-healed by update_check.py). Earlier: v6.11.20 (cloud uploads were silently dead since 2026-06-15 — two stacked bugs: a wrong-index query in `cloud_upload.py` timed out every tick, masking a second server-side unique-key collision in `snapshot.php` when two alarm registers shared a bit number; both fixed, uploads confirmed flowing again on Swanwood 2026-06-24). Earlier: v6.11.19 (data-retention timer finally installed + made safe — raised the 500MB→400MB emergency-purge cap to 20GB, gated VACUUM behind --vacuum so the nightly run can't trip the logger watchdog; time-based 30/90/365 policy). Earlier: v6.11.18 (controller-states host list from device_meta PK lookup, not a 15.8s DISTINCT scan — the iowait/overheat cure), v6.11.17 (idx_readings_host_label + 30s cache + persistent journald + SELFCHECK log). Earlier: v6.11.16 (settings-wipe root-cause fix — update_check.py chowns data back to poolai; SoC vitals in heartbeat + admin display; hourly WAL-checkpoint safety net). Earlier: v6.11.15 (UI SQLite connection-leak fix that was bloating the WAL to 5.5GB and overheating Swanwood; logger poll 5s→30s + Monitoring controls + Intensive window; System Health card). Earlier: v6.11.14 (reliability revamp — non-blocking update check, `remote_sync` retired, pre-cleanup chunk sync; PR #1 merged 2026-06-12). Earlier: v6.11.13 (screen never sleeps — labwc keep-awake + Chromium lwrespawn + Eco Mode removed; "Log Past Entry" backdated maintenance; `cloud_enabled` local-only switch), v6.11.12 (alarm-closure cloud propagation + timer self-heal).
- **Active Pis:** Swanwood (production) @ `10.0.30.247` (WiFi DHCP per 2026-04-26 recovery). New: "10inch" @ `10.0.30.148` (WiFi DHCP), a second independent pool site on the official 10" touchscreen Pi 5, fresh-installed 2026-07-30 — pool controllers not yet configured (setup wizard pending). tvcctv was decommissioned (confirmed 2026-06-11).
