...
**Last Updated:** 2026-06-15 · **Pi software current:** v6.11.16
...
- **Pi software (released):** v6.11.16 (root-cause fix for the recurring "settings wipe" — nightly root update left settings root-owned/unreadable by the poolai services; update_check.py now chowns data back to poolai. Plus SoC vitals in the heartbeat + admin display, and an hourly WAL-checkpoint safety net). Earlier: v6.11.15 (UI SQLite connection-leak fix that was bloating the WAL to 5.5GB and overheating Swanwood; logger poll 5s→30s + Monitoring controls + Intensive window; System Health card). Earlier: v6.11.14 (reliability revamp — non-blocking update check, `remote_sync` retired, pre-cleanup chunk sync; PR #1 merged 2026-06-12). Earlier: v6.11.13 (screen never sleeps — labwc keep-awake + Chromium lwrespawn + Eco Mode removed; "Log Past Entry" backdated maintenance; `cloud_enabled` local-only switch), v6.11.12 (alarm-closure cloud propagation + timer self-heal).
