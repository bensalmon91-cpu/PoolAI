...
# Installed FROM deploy/labwc_autostart (not re-declared as a heredoc here) so
# there's exactly one source of truth for the kiosk autostart. A duplicated
# copy previously drifted out of sync with the template - e.g. it missed the
# chromium_scale_factor scaling block added for the 10" touchscreen, so a Pi
# freshly cloned from this path would boot with unscaled 1:1 touch targets
# until the next update_check.py run happened to overwrite it.
if [ -f "$APP_DIR/deploy/labwc_autostart" ]; then
    sudo cp "$APP_DIR/deploy/labwc_autostart" "$PI_HOME/.config/labwc/autostart"
    sudo chown "$PI_USER:$PI_USER" "$PI_HOME/.config/labwc/autostart"
    sudo chmod +x "$PI_HOME/.config/labwc/autostart"
    echo "OK - labwc autostart installed from $APP_DIR/deploy/labwc_autostart"
else
    echo "WARNING - $APP_DIR/deploy/labwc_autostart not found, kiosk autostart NOT created"
fi
echo
