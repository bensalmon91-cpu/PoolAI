...
| `10inch` | `10.0.30.148` (WiFi, DHCP) + `192.168.200.100` (eth0) | New — independent pool site, official 10" touchscreen Pi 5. Fresh-installed 2026-07-30; pool controllers not yet configured. |
...
- `deploy/labwc_autostart`'s `chromium_scale_factor` read gained the same 0.5–4 bounds validation `persist.py` already enforces — it has no settings-page route (hand-edited JSON only), so a bad value was going straight to Chromium's command line unchecked.
- `deploy/clone_prep.sh` carried a second, hand-duplicated copy of the kiosk autostart that had already drifted out of sync with `deploy/labwc_autostart` (missing the new scale-factor block) — now installs directly from the template instead of re-declaring its contents, so the two can't diverge again.
- New `tools/tests/test_persist_chromium_scale_factor.py` closes the one real test gap the review found: range clamping, non-numeric values, the `bool`-is-`int` trap, and missing-key fallback for `chromium_scale_factor`.

**v6.11.23 (2026-07-30): USB-storage boot deadlock fixed + 10" touchscreen support.**
- Fresh-installing a new independent pool site on a 10" official touchscreen Pi ("10inch") surfaced a fleet-wide latent bug in the v6.11.22 "hardened" External Storage feature: `usb_data_mount.sh`'s `find_usb_device()` logged through a `log()` helper that wrote to stdout, and the caller captured that same stdout via `USB_DEVICE=$(find_usb_device)` — so on a Pi with **no** USB device attached, the "not found" case still returned non-empty (log) text, and the `[ -z "$USB_DEVICE" ]` "nothing to do" guard never fired. The script fell through into a full migration against `/mnt/poolaissistant_usb`, which was never actually mounted (`mount_usb()` also never checked whether `mount` itself succeeded) — so it silently "migrated" onto a plain root-fs directory instead of real external storage. Independently, and more fundamentally: the unit declares `Before=poolaissistant_logger.service poolaissistant_ui.service poolaissistant_provision.service` (so the data-directory swap finishes before they read from it), but `start_services()` called a **blocking** `systemctl start` on those same services at the end of the migration — a guaranteed self-deadlock on *any* boot that reaches that code path, real device or not, since systemd won't start them until this unit's own job completes, and this unit was blocked waiting for them. Fixed all three: `log()` now writes to stderr (journald still captures it via `StandardError=journal`, so nothing is lost from the log/journal); `mount_usb()` checks the actual `mount` exit code; `start_services()` uses `systemctl start --no-block`. Hotfixed live on Swanwood 2026-07-30 (dormant there — `poolaissistant_usb_storage.service` was present from the update but never enabled) and on 10inch (was actively hanging every boot — recovered by killing the wedged `systemctl start` call and restoring the pre-migration data directory from the `.sd_backup` the hardening step correctly preserved).
- New `chromium_scale_factor` persisted setting (same read-at-boot pattern as `screen_rotation`, wired into `deploy/labwc_autostart`) so the kiosk Chromium's `--force-device-scale-factor` can be tuned per device. Needed because the 10" touchscreen is meaningfully denser (~226dpi) than the 7" screen the UI was tuned for (~132dpi) — at 1:1 scale, touch targets would render at ~58% of their old physical size. Defaults to 1.0 (no behavior change on existing 7" units); 10inch is set to 1.5.
